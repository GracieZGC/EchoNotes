...
- 高亮优先级：**显式包裹 > 自动抽取**。
- 自动抽取规则（无包裹时触发）：
  - `State`：从判断句中抽取结论短语，剔除“关注/聚焦/围绕”等引导词。
  - `Change`：优先命中“显著增强/从…转向…”等变化片段；否则取首个有效分句。
  - `Pattern`：优先命中动词短语（如“建议/可/可以/优先/重点…”开头），并截断“的…分析/研究/评估/总结…”等尾巴。
- 长度限制：**6–24 字**；过短不高亮，过长按 `，/；/、/换行` 截断或截到最大长度。
- 回退：若无法抽取到合规片段，则不高亮或回退首分句。
...
##### 改进点 3：心情类三段去重（避免“洞察≈趋势”）
目标：让「主要洞察」与「变化趋势」分别承担**状态画像**与**变化维度**，避免重复措辞。
- **第1段（主要洞察）**：聚焦“情绪结构画像”，必须回答**开心/不开心占比或主导倾向**，并包含**触发源/情绪主题/表达方式**之一；禁止出现“相比/过往/趋势/近期变化”等词。
- **第2段（变化趋势）**：聚焦“时间变化”，必须从**早期→近期**描述变化，提到**强度/频次/波动/触发来源结构**之一，并带时间锚点（如“相比更早阶段/最近一段”）；避免复述第1段的占比或主导倾向。
- **第3段（建议）**：心情类用“留意/觉察/记录”等温和动词，写成顺畅表达（状态→可能影响→自我觉察方向），避免命令式与总结词。
补充约束（让内容更稳定、更不重复）：
- **比例词硬约束**：第1段必须出现“偏多/多数/占比更高”等比例词之一；若样本或跨度不足，可用“整体偏向尚不明显/仅能初步判断”表达。
- **方向词硬约束**：第2段必须包含变化方向词（增加/减少/趋稳/波动加大）；若难以判断趋势，可说明“趋势不明显/需要更多记录”。
- **去重约束**：第1/2段使用不同关键词，避免重复触发源或情绪主题。
- **严格报错**：心情内容不足（无法得到情绪分布/主导倾向）或 AI 解析失败时，直接报错，不走规则兜底输出。
...
### 6.3.1 图表 × 三段式洞察联动原则（基于现有代码落地）
- **同源输出，避免“图表与洞察不一致”**：图表候选与三段式洞察来自同一次 `POST /api/analysis/v3` 返回，前端不再发起二次模型调用，确保上下文一致。
- **叙事绑定（切图=切洞察）**：前端优先读取 `insightsByChartKey[chartKey]`，当用户切换图表时同步切换对应洞察；若某图缺少细分洞察，则退回通用 `insights`，保证稳定输出。
- **默认最佳图**：`charts.defaultKey` 决定首屏图表与洞察组合，保证“开箱即用”的最低成本路径。
- **证据化呈现**：图表标题直接采用 `chart.question`，作为洞察的“证据问题”；洞察负责结论与解释，图表负责量化与可视化。
- **降级兜底**：若无可用图表（或数据不足），前端展示“数据不足，仅提供文字洞察”，但三段式洞察仍然保留，保证价值输出不断档。
- **语义对齐**：建议卡标题根据 `notebookType` / `noteType` 动态切换（如监控型→“变化解读”、心情型→“自我觉察”），让三段式语义与场景类型匹配。
