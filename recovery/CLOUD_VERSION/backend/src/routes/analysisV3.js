  ...
  const suggestionGuidance = (() => {
    ...
    if (String(notebookType) === 'mood') {
      return [
        ...
        '第1段聚焦“情绪结构画像”，必须回答：开心/不开心占比或主导倾向 + 记录以什么为主（触发源/情绪主题/表达方式任选其一），且不得出现“相比/过往/趋势/近期变化”等词。',
        '第1段必须出现“偏多/多数/占比更高”等比例词之一；若样本或跨度不足以判断，可用“整体偏向尚不明显/仅能初步判断”表达。',
        '第2段聚焦“时间变化”，必须从早期到近期描述情绪变化（强度/频次/波动/触发来源结构任选其一），并带时间锚点（如：相比更早阶段/最近一段），避免复述第1段的占比或主导倾向。',
        '第2段必须包含变化方向词（如：增加/减少/趋稳/波动加大）；若难以判断趋势，可说明“趋势不明显/需要更多记录”。',
        '第1/2段用不同关键词表述，避免重复触发源或情绪主题。',
        '第3段写成顺畅表达：描述状态→可能影响→自我觉察方向，不要出现“状态：/影响：/觉察：”等总结词。',
        '第3段用“留意/觉察/记录”等温和动词，避免命令式。'
      ];
    }
    return [
